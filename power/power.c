...
#define BOOSTPULSE_SMARTASS "/sys/devices/system/cpu/cpufreq/smartass/boost_pulse"
...
static void cm_power_set_interactive(struct power_module *module, int on)
{
    if (strncmp(governor, "ondemand", 8) == 0)
        ...
        sysfs_write("/sys/devices/system/cpu/cpufreq/smartass/sample_rate_jiffies",
                on ? SAMPLING_RATE_SCREEN_ON : SAMPLING_RATE_SCREEN_OFF);
    else if (strncmp(governor, "intellidemand", 13) == 0)
        sysfs_write("/sys/devices/system/cpu/cpufreq/intellidemand/sampling_rate",
                on ? SAMPLING_RATE_SCREEN_ON : SAMPLING_RATE_SCREEN_OFF);
}
...
static void configure_governor()
{
    ...
    if (strncmp(governor, "ondemand", 8) == 0) {
        ...
        sysfs_write("/sys/devices/system/cpu/cpufreq/smartass/boost_enabled", "1");
        sysfs_write("/sys/devices/system/cpu/cpufreq/smartass/max_cpu_load", "75");
    }
}
    ...
    if (cm->boostpulse_fd < 0) {
        if (get_scaling_governor() < 0) {
            ...
            if (strncmp(governor, "ondemand", 8) == 0)
                ...
                cm->boostpulse_fd = open(BOOSTPULSE_SMARTASS, O_WRONLY);
            else if (strncmp(governor, "intellidemand", 13) == 0)
                cm->boostpulse_fd = open(BOOSTPULSE_INTELLIDEMAND, O_WRONLY);
